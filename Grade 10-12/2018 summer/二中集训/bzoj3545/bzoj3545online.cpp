...
int n,m,q,val[200010],ufs[200010],rk[200010],rak[200010];
...
int ch[4000010][2],siz[4000010],cnt,rt[200010];
int insert(int l,int r,int pre,int pos){
//	cout<<"	insert "<<l<<' '<<r<<' '<<pre<<' '<<pos<<'\n';
	int mid=(l+r)>>1,cur=++cnt;
	siz[cur]=siz[pre]+(pos!=0);ch[cur][0]=ch[pre][0];ch[cur][1]=ch[pre][1];
	if(l==r) return cur;
...
void dfs(int u,int f){
	int i,v;st[u][0]=f;seq[dfn[u]=++clk]=rak[u];
...
//	freopen("bzoj3545online.out","w",stdout);
	n=read();m=read();q=read();int i,t1,t2,t3,fx,fy,tans=0;
	w[0]=0x7fffffff;
	for(i=1;i<=n;i++) val[i]=read(),w[i]=0,ufs[i]=rk[i]=i;
	sort(rk+1,rk+n+1,cmp);
	for(i=1;i<=n;i++) rak[rk[i]]=i;
	for(i=1;i<=m;i++){
...
//	cout<<"finish build tree\n";
	dfs(root,0);ST();
	for(i=1;i<=cntp;i++) rt[i]=insert(1,n,rt[i-1],seq[i]);
//	cout<<"finish merge\n";
	for(i=1;i<=q;i++){
		t1=read();t2=read();t3=read();
		t1^=tans;t2^=tans;t3^=tans;
		getp(t1,t2);
//		cout<<"finish getp "<<t1<<'\n';
		int re=query(1,n,rt[out[t1]],rt[dfn[t1]-1],t3);
		if(~re) printf("%d\n",val[rk[re]]),tans=val[rk[re]];
		else puts("-1");
	}
}
